  ...
  useEffect(() => {
    ...
    const initApp = async () => {
      try {
        ...
        // Añadir canvas al contenedor — escalar CSS para responsividad
    ...
    const initWorld = () => {
      ...
      // Iniciar game loop
      app.ticker.add((ticker: any) => gameLoop(ticker.deltaMS))
    }
    ...
    const drawLaboratory = () => {
      ...
      // Floor (PixiJS v8 no soporta gradientes en Graphics — usamos fill sólido)
      bg.rect(0, 0, WORLD_WIDTH, WORLD_HEIGHT)
      bg.fill({ color: 0x0f1428 })
    ...
    const createRobots = () => {
      ...
      for (const station of STATION_POSITIONS) {
        ...
        worldRef.current.addChild(robot as any)
      }
    }
    ...
    const createCoordinator = () => {
      ...
      worldRef.current.addChild(coordinator as any)
    }
