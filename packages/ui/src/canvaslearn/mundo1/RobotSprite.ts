...
export class RobotSprite extends Container {
  ...
  private createComponents(): void {
    ...
    this.nameLabel.y = config.bodyHeight / 2 + config.headRadius + 8
    ...
    this.roleLabel.y = this.nameLabel.y + 14
   ...
   */
  showSpeechBubble(type: 'running' | 'completed' | 'failed' | 'thinking' | 'greet' | 'celebrate'): void {
  ...
  showXPBar(currentXP: number, maxXP: number, level: number): void {
    ...
    // Nivel
    if (this.nameLabel) {
      this.nameLabel.text = `Lv.${level} ${AGENT_LABELS[this.agentId]?.label || ''}`
    }
  }
